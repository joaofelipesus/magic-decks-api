defmodule MagicDecks.MagicApi.ClientTest do
  use ExUnit.Case
  import Tesla.Mock
  alias MagicDecks.MagicApi.Client

  @request_url "https://api.magicthegathering.io/v1/cards?name=Akroma,%20Anjo%20da%20Ira&language=Portuguese%20(Brazil)"

  def response_body do
    %{
      "cards" => [
        %{
  ...
  describe "find_by_name/2" do
    test "returns a list with many elements when name matches with more than one card" do
      mock(fn %{
                method: :get,
                url: @request_url
              } ->
    ...
    test "returns magic cards, when search using english name" do
      mock(fn %{
                method: :get,
                url: "https://api.magicthegathering.io/v1/cards?name=Akroma,%20Angel%20of%20Wrath"
              } ->
        %Tesla.Env{status: 200, body: response_body()}
      end)
    ...
    test "returns error message, when api returns error status" do
      mock(fn %{method: :get, url: @request_url} ->
        {:error, :timeout}
      end)

      response = Client.find_by_name("Akroma, Anjo da Ira", lang: :pt)
